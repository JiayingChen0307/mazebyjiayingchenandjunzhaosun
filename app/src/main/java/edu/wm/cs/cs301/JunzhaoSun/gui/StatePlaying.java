...
import edu.wm.cs.cs301.JunzhaoSun.generation.Floorplan;
...
public class StatePlaying extends DefaultState {
    ...
    Map mapView;
    MazePanel panel;
    PlayManuallyActivity mActivity;
    PlayAnimationActivity mActivity2;
    Maze mazeConfig;
    Robot robot;
    RobotDriver driver;
    ...
    public void setActivity(PlayManuallyActivity mActivity){ this.mActivity = mActivity;}

    public void setActivity(PlayAnimationActivity mActivity2){ this.mActivity2 = mActivity2;}
    ...
    public void start(MazePanel panel) {
        ...
        //set robot
        robot = mActivity == null ? mActivity2.robot : mActivity.robot;
    ...
    protected void startDrawer() {
        ...
        firstPersonView.setPanel(panel);
     ...
     * Method requires {start(MazePanel) start} to be
     * called before.
     *
    ...
    public boolean keyDown(UserInput key, int value) {
        ...
        switch (key) {
            ...
            case Up: // move forward
                ...
                if (isOutside(px, py)) {
                    robot.setWin();
                    if (mActivity != null)
                        mActivity.switchToWinning();
                    else
                        mActivity2.switchToWinning();
                }
                break;
            case Left: // turn left
                rotate(1);
        ...
        if (robot.hasStopped()) {
            // if the robot runs out of energy, end the game
            if (mActivity != null)
                mActivity.switchToLosing();
            else
                mActivity2.switchToLosing();
        }
        return true;
    }
    ...
    protected void draw() {
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                synchronized (panel) {
                    panel.update();
                    remainText.setText("Remaining energy : " + robot.getBatteryLevel());
                    remainBar.setProgress((int)robot.getBatteryLevel());
                    panel.notify();
                }
            }
        });
    ...
    public void setTextBar(TextView textView, ProgressBar progressBar, Handler handler){
