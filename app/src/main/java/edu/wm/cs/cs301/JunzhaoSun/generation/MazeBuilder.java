...
import edu.wm.cs.cs301.JunzhaoSun.gui.Constants;
...
public class MazeBuilder implements Runnable {
    ...
    public void buildOrder(Order order) {
        ...
        floorplan.initialize();//change
    }
    public Floorplan getFloorPlan() {//new method
    ...
    public void run() {
        ...
        try {
            ...
            final Maze mazeConfig = new MazeContainer();
            mazeConfig.setHeight(height);
            mazeConfig.setWidth(width);
            mazeConfig.setFloorplan(floorplan);
            ...
            order.getHandler().post(new Runnable() {
                @Override
                public void run() {
                    order.getProgressBar().setProgress(100);
                }
            });
