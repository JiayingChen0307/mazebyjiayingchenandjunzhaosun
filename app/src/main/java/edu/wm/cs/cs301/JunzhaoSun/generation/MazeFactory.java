...
public class MazeFactory implements Factory {
	...
	public boolean order(Order order) {
		...
		switch (order.getBuilder()) {
			case DFS :
				builder = deterministic? new MazeBuilder(true) : new MazeBuilder(seed);
				buildOrder();
				break;
			case Prim:
				builder = deterministic? new MazeBuilderPrim(true) : new MazeBuilderPrim(seed);
				buildOrder();
				break;
			case Eller:
				builder = deterministic? new MazeBuilderEller(true) : new MazeBuilderEller(seed);
				buildOrder();
				break;
			default:
